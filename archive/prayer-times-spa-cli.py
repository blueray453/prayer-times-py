#!/usr/bin/env python3
"""
Hadith-Based Islamic Prayer Time Calculator — v20 NREL SPA
----------------------------------------------------------
...
Solar position is computed with NREL SPA (via pvlib) instead of Meeus,
for higher accuracy. Requires: pip install pandas pytz pvlib
...
def solar_position_spa(date, lat, lon, tz_offset, elevation=0.0, temperature=15.0, pressure_mbar=1013.25):
    ...
    eot_deg = eot_min * 0.25
    return decl_deg, eot_deg
...
def calculate(y, m, d, lat, lon, tz, fajr_angle, isha_angle, asr_ratio, asr_early_fraction, elevation, temperature,
              pressure):
...
def main():
    p = argparse.ArgumentParser(
        ...
        "+ NREL SPA solar position). Fajr/Isha computed by PCD "
        "unless overridden.",
