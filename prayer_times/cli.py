#!/usr/bin/env python3
"""
Hadith-Based Islamic Prayer Time Calculator (CLI) — v23 UNIFIED
---------------------------------------------------------------
One script, three modes:

  Default    →  Fixed angles (--fajr-angle and --isha-angle REQUIRED)
  --pcd      →  PCD dynamic angles, Meeus solar solver
  --spa      →  PCD dynamic angles, NREL SPA solar solver (needs pvlib)

Mode rules (strict):
  • Fixed mode: --fajr-angle and --isha-angle are REQUIRED.
  • --pcd / --spa: --fajr-angle and --isha-angle are NOT ALLOWED.
    Supplying either one with --pcd or --spa is a hard error.
    PCD computes both angles; there is no override path.

The three forbidden intervals follow Sahih Muslim 831 exactly:
  1. sun begins to rise → fully up
  2. sun at height → passes meridian
  3. sun draws near to setting → sets

Usage:
  # Fixed 18°/17° (MWL convention)
...
# ============================================================
# PCD DYNAMIC TWILIGHT ANGLE ALGORITHM
# ============================================================
...
def get_pcd_angles(d, lat, lng=0.0, elevation=0.0, temperature=15.0, pressure=1013.25, shafaq="general"):
    ...
    decl, r, ecl_lon = _solar_ephemeris_pcd(jd)
...
# ============================================================
# SOLAR POSITION — Meeus (default) and NREL SPA (--spa)
# ============================================================
...
def solar_position_spa(date, lat, lon, tz_offset, elevation=0.0, temperature=15.0, pressure_mbar=1013.25):
    """NREL SPA via pvlib. Imported lazily on first call."""
    global _pd, _pytz, _solarposition
    try:
        _pd
    except NameError:
        try:
            ...
            from pvlib import solarposition as _solarposition
        except Exception as e:
            sys.exit(f"[FATAL] --spa requires pvlib/pandas/pytz: {e}\n"
                     f"        Run: pip install pandas pytz pvlib")

    local_tz = _pytz.FixedOffset(int(round(tz_offset * 60)))
    noon_local = _pd.Timestamp(
        year=date.year,
        month=date.month,
        day=date.day,
        hour=12,
    ...
    times = _pd.DatetimeIndex([noon_local])

    spa = _solarposition.get_solarposition(
        times,
        latitude=lat,
        longitude=lon,
        altitude=elevation,
...
def calculate(y, m, d, lat, lon, tz, fajr_angle, isha_angle, asr_ratio, asr_early_fraction, mode, elevation,
    ...
    if mode == "spa":
        ...
        jd = julian_day(y, m, d)
        decl, EoT = solar_position_meeus(jd)
...
def print_report(r, a, tz_label, mode, fajr_src, isha_src):
    mode_label = {
        "fixed": "FIXED ANGLES",
        "pcd": "PCD DYNAMIC",
        "spa": "PCD DYNAMIC + NREL SPA",
    }[mode]

    line = "=" * 84
    print(f"\n{line}")
    print(f"  HADITH-BASED PRAYER TIMES  —  {VERSION}  [{mode_label}]")
    print(f"{line}")
    print(f"  Date       : {a.date}")
    print(f"  Location   : {a.lat:.4f}°, {a.lon:.4f}°   Elev: {a.elev} m")
    print(f"  Timezone   : {tz_label}  (UTC{a.tz_offset:+.1f})")
    print(f"  Angles     : Fajr={a.fajr_angle}° ({fajr_src})  "
          f"Isha={a.isha_angle}° ({isha_src})")
...
def main():
    p = argparse.ArgumentParser(
        description="Hadith-based prayer time calculator. "
        "Three modes: fixed angles (default), --pcd, --spa. "
        "--fajr-angle/--isha-angle are mutually exclusive with "
        "--pcd/--spa.",
    ...
    mode_group.add_argument("--pcd",
                            action="store_true",
                            help="Use PCD dynamic angles with the Meeus "
                            "solar solver. --fajr-angle and --isha-angle "
                            "are NOT ALLOWED with this flag.")
    mode_group.add_argument("--spa",
                            action="store_true",
                            help="Use PCD dynamic angles with the NREL SPA "
                            "solar solver (requires pvlib/pandas/pytz). "
                            "--fajr-angle and --isha-angle are NOT "
                            "ALLOWED with this flag.")

    # --- Angles ---
    p.add_argument("--fajr-angle",
                   type=float,
                   default=None,
                   help="Fajr solar DEPRESSION in degrees (positive). "
                   "REQUIRED in fixed mode. NOT ALLOWED with --pcd/--spa.")
    p.add_argument("--isha-angle",
                   type=float,
                   default=None,
                   help="Isha solar DEPRESSION in degrees (positive). "
                   "REQUIRED in fixed mode. NOT ALLOWED with --pcd/--spa.")
    ...
    p.add_argument("--asr-early-fraction",
                   type=float,
                   default=0.33,
                   help="Fraction of the Asr preferred window treated as "
                   "'best'. 0.0 = opening moment only; 1.0 = whole window.")

    # --- PCD environment (ignored in fixed mode) ---
    p.add_argument("--temperature",
                   type=float,
                   default=15.0,
                   help="Ambient temperature (°C) for PCD refraction. "
                   "Ignored in fixed mode.")
    p.add_argument("--pressure",
                   type=float,
                   default=1013.25,
                   help="Atmospheric pressure (mbar) for PCD refraction. "
                   "Ignored in fixed mode.")
    p.add_argument("--shafaq",
                   type=str,
                   default="general",
                   choices=["general", "ahmer", "abyad"],
                   help="Isha twilight mode for PCD. Ignored in fixed mode.")
    ...
    # --- Strict validation: angles are mutually exclusive with --pcd/--spa ---
    if mode == "fixed":
        if a.fajr_angle is None:
            p.error("--fajr-angle is required in fixed mode "
                    "(it is NOT ALLOWED with --pcd or --spa)")
        if a.isha_angle is None:
            p.error("--isha-angle is required in fixed mode "
                    "(it is NOT ALLOWED with --pcd or --spa)")
    else:
        if a.fajr_angle is not None:
            p.error(f"--fajr-angle is NOT ALLOWED with --{mode}. "
                    f"PCD computes the Fajr angle dynamically; there is no "
                    f"override. Use fixed mode (remove --{mode}) if you want "
                    f"to supply the angle manually.")
        if a.isha_angle is not None:
            p.error(f"--isha-angle is NOT ALLOWED with --{mode}. "
                    f"PCD computes the Isha angle dynamically; there is no "
                    f"override. Use fixed mode (remove --{mode}) if you want "
                    f"to supply the angle manually.")

    # --- Angle range validation (only reachable in fixed mode) ---
    ...
    # --- Resolve Fajr / Isha angles ---
    if mode == "fixed":
        fajr_angle = a.fajr_angle
        isha_angle = a.isha_angle
        fajr_src = "fixed"
        isha_src = "fixed"
    else:
        fajr_angle, isha_angle = get_pcd_angles(
            d,
            lat=a.lat,
            lng=a.lon,
            elevation=a.elev,
    ...
    # Store resolved values back on namespace for print_report
    a.fajr_angle = fajr_angle
    a.isha_angle = isha_angle
    ...
    print_report(r, a, tz_label, mode, fajr_src, isha_src)
